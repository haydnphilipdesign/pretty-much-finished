...
const ContentCard: React.FC<ContentCardProps> = ({
  ...
  let baseCardStyle = 'rounded-2xl overflow-hidden transition-all duration-300 max-w-full relative w-full';
  ...
  if (heroStyle) {
    ...
    baseCardStyle += ' bg-white border border-gray-100 p-4 sm:p-6 shadow-md hover:shadow-lg';
  }

  // Additional inline styles for backdrop-filter
  const cardInlineStyle = heroStyle ? {
  ...
  const titleStyle = heroStyle
    ? 'text-lg sm:text-xl font-bold text-white mb-3 sm:mb-4 break-words'
  ...
  const subtitleStyle = heroStyle
    ? 'text-xs sm:text-sm text-blue-200 mb-3 sm:mb-4'
    : 'text-xs sm:text-sm text-gray-600 mb-3 sm:mb-4 italic';
