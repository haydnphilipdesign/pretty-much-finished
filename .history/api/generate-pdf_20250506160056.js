        ...
        try {
            ...
            if (formData.agentData) {
                ...
                const agentName = formData.agentData.name ||
                    ...
                    (formData.signatureData ? formData.signatureData.agentName : null) ||
                    (formData.signatureData ? formData.signatureData.signature : null) ||
                    'Unknown Agent';
            ...
            if (formData.clients && formData.clients.length > 0) {
                ...
                const buyers = normalizedClients.filter(c =>
                    c.type ? c.type.includes('BUYER') : false);

                const sellers = normalizedClients.filter(c =>
                    c.type ? c.type.includes('SELLER') : false);
