...
const clientFieldMap = {
  ...
  clientAddress: 'fldz1IpeR1256LhuC', // Original client address field
  address: 'fldx7IEsPmHTJXDYS',      // Address field that matches with transactions table
...
async function submitClientToAirtable(client: any) {
  ...
  // Add address to both address fields for consistency
  if (client.address) {
    clientFields[clientFieldMap.clientAddress] = client.address; // Original client address field
    clientFields[clientFieldMap.address] = client.address;       // Address field for automation
  }
...
export async function submitToAirtable(data: TransactionFormState, pdfUrl?: string) {
  const transactionFields: { [key: string]: any } = {};

  try {
    // Add PDF attachment if available
    if (pdfUrl) {
      transactionFields[transactionFieldMap.pdfAttachment] = [{ url: pdfUrl }];
    }
    ...
    const clientPromises = data.clients.map(client => submitClientToAirtable(client));
