...
export const useTransactionForm = () => {
  ...
  const handleNext = () => {
    ...
    if (Object.keys(errors).length === 0 || bypassValidationMode) {
      ...
      // Show a toast with validation errors and a proceed anyway button 
      toast({
      ...
      // Also show a separate toast with the bypass option
      toast({
        title: "Continue Anyway?",
        description: "You can proceed with incomplete information and provide it later",
        variant: "default",
        action: <button 
          onClick={() => {
