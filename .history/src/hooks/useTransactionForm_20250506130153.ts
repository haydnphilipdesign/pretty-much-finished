  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      ...
      try {
        ...
        // Get the PDF base64 data, checking both possible response properties
        const base64Data = pdfResult.pdfBase64 || pdfResult.pdfData;
        ...
        try {
          ...
          if (!uploadResult.success) {
            console.warn(`Supabase upload failed: ${uploadResult.error}. Trying direct Airtable attachment...`);
            // If Supabase fails, try direct Airtable attachment as fallback
            const attachmentResult = await formatPdfForAirtable(base64Data, filename, result.transactionId);
            
            if (!attachmentResult) {
              throw new Error('Failed to attach PDF to Airtable directly');
            }
            
            console.log("PDF attached to Airtable via direct API.");
            toast({
              title: "PDF Attached",
              description: "Transaction PDF has been attached to record.",
              variant: "default",
            });
          } else {
        ...
        // Send email with the PDF regardless of upload status
