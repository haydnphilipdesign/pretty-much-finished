...
// Configuration
const PRODUCTION_URL = process.env.PRODUCTION_URL || 'https://pa-real-estate-support-services-bpcxvdeqw.vercel.app';
...
async function testEndpoint(endpoint) {
    ...
    try {
        ...
        // Prepare headers with bypass secret if available
        const headers = {
            'Content-Type': 'application/json'
        };

        // Add the bypass secret if it's available
        if (VERCEL_BYPASS_SECRET) {
            headers['bypass_secret'] = VERCEL_BYPASS_SECRET;
            console.log('Using Vercel bypass secret for authentication');
        } else {
            console.log('Warning: No bypass secret found in environment variables');
        }
...
async function runTests() {
    ...
    if (!allCorrect) {
        ...
        console.log('6. Ensure VERCEL_AUTOMATION_BYPASS_SECRET is set in your environment variables');
    }
}
