...
const SmoothLink: React.FC<SmoothLinkProps> = ({
  ...
  // Create custom fade-out animation when navigating
  useEffect(() => {
    if (!isNavigating) return;

    // Add transition styles to hero content elements when navigating
    const heroElements = document.querySelectorAll('[data-hero-content]');
    heroElements.forEach(el => {
      if (el instanceof HTMLElement) {
        el.style.transition = `opacity ${fadeOutDuration}ms ease-out, transform ${fadeOutDuration}ms ease-out`;
        el.style.opacity = '0';
    ...
    return () => {
      // Reset transition styles when component unmounts
      heroElements.forEach(el => {
        if (el instanceof HTMLElement) {
          el.style.transition = '';
          el.style.opacity = '';
