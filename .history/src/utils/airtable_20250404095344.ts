...
export const submitToAirtable = async (data: TransactionFormData) => {
  try {
    ...
    // Format addresses before submission if they're not already formatted
    const propertyAddress = data.propertyData.address || formatAddress(
      data.propertyData.streetAddress,
      data.propertyData.city,
      data.propertyData.state || "PA",
      data.propertyData.zipCode
    );
    
    const formattedClients = data.clients.map(client => {
      const clientAddress = client.address || formatAddress(
    ...
    const clientPromises = data.clients.map(async (client) => {
      // Now we're just using the address directly
      ...
      const fields = {
        ...
        [clientFieldMap.clientType]: client.type,
        // Other client fields
      };
    ...
    const pdfUrl = await generateCoverSheetForTransaction(
