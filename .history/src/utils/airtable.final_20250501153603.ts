...
const clientFieldMap = {
  ...
  relatedTransactions: 'fldYsIpMRHmvRjpUd'
};
...
async function submitClientToAirtable(client: any, transactionId: string) {
  ...
  // Add the related transaction ID as a linked record
  if (transactionId) {
    clientFields[clientFieldMap.relatedTransactions] = [{ id: transactionId }];
  }
...
export async function submitToAirtable(data: TransactionFormState) {
  ...
  try {
    ...
    // Now submit each client record with the transaction ID
    const clientPromises = data.clients.map(client => submitClientToAirtable(client, transactionId));
