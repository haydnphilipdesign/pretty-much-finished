...
export const formFieldPositions = {
  ...
  page1: {
    ...
    salePrice: { x: 278.45, y: 690.52 + 2 + 7.6, fontSize: 11, bold: true },
    sellersAssist: { x: 396.44, y: 690.52 + 2 + 7.6, fontSize: 11, bold: true },
    
    // Closing Information section - Move UP by 0.65 points (half of 1.3)
    closingDate: { x: 536.28, y: 644.28 + 2 + 0.65, fontSize: 10, bold: true },
    
    // Agent Information section - Move DOWN by 7.4/7.05 points (half of 14.8/14.1)
    agentName: { x: 96.55, y: 546.91 + 2 - 7.4, fontSize: 11, bold: true },
    agentRole: { x: 269.44, y: 546.91 + 2 - 7.05, fontSize: 11, bold: true },
    
    // Buyer Information section - Move DOWN by half the specified amounts
    buyerName: { x: 143.17, y: 494.06 + 2 - 14.9, fontSize: 11, bold: true },
    buyerPhone: { x: 135.43, y: 467.63 + 2 - 10.9, fontSize: 11 },
    buyerAddress: { x: 81.46, y: 441.20 + 2 - 7.4, fontSize: 11 },
    buyerEmail: { x: 125.10, y: 414.77 + Y_OFFSET - 0, fontSize: 11 },
    
    // Seller Information section - Move DOWN by half the specified amounts
    sellerName: { x: 423.75, y: 494.06 + 2 - 2, fontSize: 11, bold: true },
    sellerPhone: { x: 420.21, y: 467.63 + 2 - 2, fontSize: 11 },
    sellerAddress: { x: 370.00, y: 441.20 + 2 - 2, fontSize: 11 }, // Shifted left from 420.00
    sellerEmail: { x: 408.98, y: 414.77 + Y_OFFSET - 0, fontSize: 11 },
    
    // Legal Information section - Move DOWN by 13.9 points (half of 27.8)
    attorneyName: { x: 94.97, y: 361.92 + 2 - 2, fontSize: 11 },
    
    // Financial Details section - Move DOWN by half the specified amounts
    listSidePercentage: { x: 113.13, y: 289.24 + 2 - 2, fontSize: 11, bold: true },
    buyerSidePercentage: { x: 251.81, y: 289.24 + 2 - 2, fontSize: 11, bold: true },
    totalPercentage: { x: 113.13, y: 262.81 + 2 - 2, fontSize: 11, bold: true },
    
    // Title Information - Move DOWN by 14.9 points (half of 29.8)
    titleCompany: { x: 420.59, y: 163.71 + 2 - 2, fontSize: 11, bold: true },
    
    // Referral Information section - Move DOWN by 14.9 points (half of 29.8)
    referralDueTo: { x: 108.12, y: 57.99 + 2 - 14.9, fontSize: 11, bold: true },
    referralPercentage: { x: 264.81, y: 57.99 + 2 - 14.9, fontSize: 11, bold: true },
  }
};
