...
export const generateTestData = (role: AgentRole) => {
  ...
  const testWarrantyData: WarrantyData = {
    hasWarranty: randomBoolean(),
    provider: randomBoolean() ? randomElement(warrantyCompanies) : "",
    cost: randomBoolean() ? `${randomInt(400, 800)}` : "",
    paidBy: randomBoolean() ? "seller" : "buyer",
  };
  ...
  const testCommissionData: CommissionData = {
    ...
    buyerPaidCommission: randomBoolean() ? randomPercentage() : "0",
    ...
    brokerSplit: randomBoolean() ? randomPercentage() : "",
  ...
  return {
    ...
    commissionData: {
      ...testCommissionData,
      commissionBase: testCommissionData.commissionBase,
    },
