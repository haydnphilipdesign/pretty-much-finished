...
export function TransactionForm() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Silently handle PDF generation in the background
      try {
        await sendTransactionPdfViaApi(formData);
      } catch (error) {
        console.error('PDF generation error:', error);
        // Don't show PDF errors to the user
      }
      
      // Show success message and reset form
      toast({
        title: "Success!",
        description: "Transaction submitted successfully. The form has been reset for a new submission.",
        variant: "default",
      });
      ...
      if (result?.success) {
        ...
        try {
          ...
          // Add a small delay to ensure Airtable submission is complete
          await new Promise(resolve => setTimeout(resolve, 500));
          
          // Show toast for PDF generation
          toast({
            title: "Creating PDF...",
            description: "Generating transaction PDF document",
            variant: "default",
          });
          
          // Add the transaction ID to the form data for PDF generation
          const pdfFormData = {
            ...formData,
            transactionId: result.transactionId  // Add transaction ID for Airtable attachment
          };
          
          // Call the local PDF API endpoint
          const pdfResult = await sendTransactionPdfViaApi(pdfFormData);
          console.log("PDF generation result:", pdfResult);
          
          // Show success toast
          toast({
            title: "PDF Generated",
            description: "Transaction PDF has been generated and sent via email.",
            variant: "default",
          });
        } catch (pdfError) {
          ...
          toast({
            title: "PDF Warning",
            description: "Transaction was saved but there was an issue generating the PDF. Please contact support and mention: " + (pdfError.message || 'Unknown PDF error'),
            variant: "destructive",
          });
