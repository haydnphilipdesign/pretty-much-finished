...
export const useTransactionForm = () => {
  ...
  const handleNext = () => {
    ...
    if (Object.keys(errors).length === 0 || bypassValidationMode) {
      ...
      // Show a second toast with option to proceed anyway
      toast({
        title: "Continue Anyway?",
        description: "Click 'Skip Validation' below to proceed with incomplete information",
        variant: "default",
      });
      
      // Add a "Proceed Anyway" button to the page temporarily
      const skipBtn = document.createElement('button');
      skipBtn.innerText = 'Skip Validation';
      skipBtn.className = 'fixed bottom-4 right-4 z-50 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-md';
      skipBtn.onclick = () => {
        setBypassValidationMode(true);
        setTimeout(() => handleNext(), 100); // Small delay to ensure state is updated
