...
function mapFormDataToPositions(formData) {
    ...
    if (formData.agentData) {
        ...
        // Replace optional chaining with traditional approach
        const role = formData.agentData.role && formData.agentData.role.toUpperCase ?
            formData.agentData.role.toUpperCase() :
            '';

        addTextElement(textElements, 0, formFieldPositions.page1.agentRole, role);
    }
    ...
    if (formData.clients && formData.clients.length > 0) {
        ...
        const buyers = formData.clients.filter(c => {
            ...
            return c.type && (
                c.type.toLowerCase && c.type.toLowerCase() === 'buyer' ||
                c.type.toLowerCase && c.type.toLowerCase() === 'buyers'
            );
        });
        ...
        const sellers = formData.clients.filter(c => {
            // Replace optional chaining with traditional checks
            return c.type && (
                c.type.toLowerCase && c.type.toLowerCase() === 'seller' ||
                c.type.toLowerCase && c.type.toLowerCase() === 'sellers'
            );
        });
...
// Export the functions and constants for CommonJS
module.exports = {
    formFieldPositions,
    mapFormDataToPositions,
    addTextElement
};