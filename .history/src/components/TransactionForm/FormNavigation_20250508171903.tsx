    ...
    <div className="flex flex-col gap-4 md:gap-6">
          ...
          >
            <button
              ...
              className={`
                ...
                ${currentStep === step
                  ? "bg-gradient-to-r from-brand-gold to-brand-blue text-white" 
                  : currentStep > step
                  ? "bg-blue-200 text-blue-800"
                  : "bg-gray-200 text-gray-500"}
                ${onStepClick ? "cursor-pointer hover:ring-2 hover:ring-brand-gold/50" : "cursor-default"}
              `}
            >
              {step}
              ...
              {stepHasMissingFields(step) && (
                <span className="absolute -top-1 -right-1 flex h-4 w-4">
                  ...
                  <span className="relative inline-flex rounded-full h-4 w-4 bg-amber-500 text-white text-[9px] items-center justify-center">
            ...
            {step < totalSteps && (
              <div 
                className={`hidden md:block h-0.5 w-full mt-5 ${
                  currentStep > step ? "bg-brand-gold/50" : "bg-gray-200"
                }`}
                style={{ 
                  width: '100%',
                  position: 'absolute',
      ...
      <div className="flex justify-between w-full mt-2">
        ...
        {currentStep < totalSteps ? (
          <Button onClick={onNext} className="bg-gradient-to-r from-brand-gold to-brand-blue hover:shadow-md text-white">
            Next
          ...
          <Button 
            ...
            className="bg-gradient-to-r from-brand-gold to-brand-blue hover:shadow-md text-white"
          >
            {isSubmitting ? (
              <motion.div
                animate={{ rotate: 360 }}
