...
export function useFormValidation() {
  ...
  const validateField = useCallback((fieldName: string, value: any, context?: Record<string, any>, isRequired?: boolean): string | null => {
    ...
    if (fieldName.includes('phone') && typeof value === 'string') {
      // Allow for formatting characters but ensure 10 digits
    ...
    if ((fieldName.includes('AgentCommission') || fieldName.includes('referralFee')) &&
        typeof value === 'string') {
    ...
    if (fieldName === 'mlsNumber') {
      const mlsRegex = /^(?:\d{6}|PM-\d{6})$/;
      if (!mlsRegex.test(value)) {
        return 'MLS number must be either 6 digits or PM-xxxxxx format';
      }
    }
    // License number validation
    if (fieldName.includes('License') && value) {
      const licenseRegex = /^[A-Z]{2}\d{6,}$/;
      if (!licenseRegex.test(value)) {
        return 'License number should be in format XX123456';
      }
    }
    // EIN validation
    if (fieldName === 'brokerEin' && value) {
      const einRegex = /^\d{2}-\d{7}$/;
      if (!einRegex.test(value)) {
        return 'EIN should be in format XX-XXXXXXX';
      }
    }
    // Date validation
    ...
    Object.entries(state.formData).forEach(([section, sectionData]) => {
      ...
      if (typeof sectionData === 'object' && sectionData !== null) {
        ...
        Object.entries(sectionData as Record<string, any>).forEach(([field, value]) => {
          ...
          const isRequired = isFieldRequired(section, field, sectionData as Record<string, any>, state.selectedRole);
        ...
        sectionData.forEach((item, index) => {
          if (typeof item === 'object' && item !== null) {
            Object.entries(item).forEach(([field, value]) => {
              ...
              // Check if field is required
              const isRequired = isFieldRequired(section, field, item, state.selectedRole);
    ...
    if (typeof sectionData === 'object' && sectionData !== null) {
      if (Array.isArray(sectionData)) {
        ...
        sectionData.forEach((item, index) => {
          if (typeof item === 'object' && item !== null) {
            Object.entries(item).forEach(([field, value]) => {
              ...
              // Check if field is required
              const isRequired = isFieldRequired(sectionName, field, item, state.selectedRole);
        ...
        Object.entries(sectionData as Record<string, any>).forEach(([field, value]) => {
          ...
          // Check if field is required
          const isRequired = isFieldRequired(sectionName, field, sectionData as Record<string, any>, state.selectedRole);
