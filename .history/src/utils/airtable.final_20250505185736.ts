  ...
  try {
    ...
    if (pdfBuffer) {
      const addressSlug = data.propertyData?.address
        ? data.propertyData.address.replace(/[^a-zA-Z0-9]/g, '_').replace(/_+/g, '_').substring(0, 30)
        : 'unknown_address';
      ...
      if (formattedAttachment) {
        transactionFields[transactionFieldMap.pdfAttachment] = formattedAttachment;
      } else {
        transactionFields[transactionFieldMap.additionalNotes] = (transactionFields[transactionFieldMap.additionalNotes] || '') + '\nPDF could not be attached due to size or upload error.';
      }
    }
