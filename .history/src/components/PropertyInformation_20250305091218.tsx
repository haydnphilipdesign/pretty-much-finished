...
export function PropertyInformation({ role }: PropertyInformationProps) {
  const { state, dispatch } = useTransactionForm();
  const data = state.formData.propertyData || {
    mlsNumber: '',
    address: '',
    salePrice: '',
    status: '',
  ...
  const handleMlsNumberChange = (value: string) => {
    // Allow PM- prefix and digits
    let cleaned = value.replace(/[^0-9PM-]/g, '');
  
    // Handle PM- prefix
    if (cleaned.startsWith('PM-')) {
      // Keep PM- prefix and up to 6 digits after
      cleaned = 'PM-' + cleaned.slice(3).replace(/\D/g, '').slice(0, 6);
    } else {
      // For non-PM numbers, just keep up to 6 digits
      cleaned = cleaned.replace(/\D/g, '').slice(0, 6);
    }
  
    handleChange("mlsNumber", cleaned);
  };
      ...
      <Card className="p-6 backdrop-blur-lg bg-white/30 border-white/30 text-slate-800">
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <RadioGroup value={data?.status || ''} onValueChange={value => handleChange("status", value)} className="flex space-x-6">
          ...
          {data?.status === "vacant" && (
            <div className="flex items-center space-x-2">
              <Switch 
                ...
                onCheckedChange={checked => handleChange("isWinterized", checked)} 
              />
          ...
          {canUpdateMls && (
            <div className="flex items-center space-x-2">
              <Switch 
                ...
                onCheckedChange={checked => handleChange("updateMls", checked)} 
              />
