...
const ContentCard: React.FC<ContentCardProps> = ({
  ...
  const cardStyle = heroStyle
    ? 'hero-card relative p-8'
    : dark
    ? 'content-card-dark relative p-8'
    : 'content-card relative p-8';
  ...
  const titleStyle = heroStyle
    ? 'hero-card-title mb-4'
    : 'text-xl font-bold text-brand-blue mb-4';

  // Determine subtitle styling based on props
  const subtitleStyle = heroStyle
    ? 'text-sm text-blue-200 mb-6'
