...
function mapFormDataToPositions(formData) {
    ...
    if (formData.agentData) {
        ...
        const role = formData.agentData.role && typeof formData.agentData.role === 'string' &&
            formData.agentData.role.toUpperCase ? formData.agentData.role.toUpperCase() : '';
    ...
    if (formData.clients && formData.clients.length > 0) {
        ...
        const sellers = formData.clients.filter(c => {
            ...
            const type = String(c.type).toLowerCase();
            return type === 'seller' || type === 'sellers';
        });
