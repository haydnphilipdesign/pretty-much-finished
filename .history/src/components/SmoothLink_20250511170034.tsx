...
const SmoothLink: React.FC<SmoothLinkProps> = ({
  ...
  const handleClick = useCallback((e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    // DIRECT APPROACH: Use native browser scrolling with smooth behavior
    try {
      // First, try to use the most compatible method
      window.scroll({
        top: 0,
        left: 0,
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        // Force scroll one more time right before navigation
        window.scrollTo(0, 0);
    ...
    }, 50); // Short delay to ensure the first scroll takes effect
