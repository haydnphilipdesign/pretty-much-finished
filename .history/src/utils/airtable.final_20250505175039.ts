      ...
      try {
        ...
        if (pdfBuffer.length > MAX_PDF_SIZE_BYTES) {
          ...
          try {
            ...
            let bufferToCompress = pdfBuffer;
            if (!(pdfBuffer instanceof Buffer)) {
              bufferToCompress = Buffer.from(pdfBuffer);
            }
            const compressedPdf = await compressPdfBuffer(bufferToCompress, MAX_PDF_SIZE_BYTES);
            if (compressedPdf) {
              ...
              const finalPdfBuffer = compressedPdf.length <= MAX_PDF_SIZE_BYTES 
                ? compressedPdf 
                : Buffer.isBuffer(compressedPdf) ? compressedPdf.subarray(0, MAX_PDF_SIZE_BYTES - 1000) : Buffer.from(compressedPdf).subarray(0, MAX_PDF_SIZE_BYTES - 1000);
