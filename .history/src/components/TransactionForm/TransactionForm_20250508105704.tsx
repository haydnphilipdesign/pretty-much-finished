...
export function TransactionForm() {
  ...
  const resetForm = () => {
    ...
    // Close the reset confirmation dialog - force it closed
  ...
  useEffect(() => {
    if (formContentRef.current) {
      ...
      // Account for the sticky header height and navigation elements
      window.scrollTo({
        top: formContentRef.current.offsetTop - 240, // Increased offset to ensure title is fully visible
        behavior: 'smooth'
      });
    }
  }, [currentStep]);
                    ...
                    <DialogFooter className="flex space-x-2 mt-4">
                      <Button 
                        ...
                        onClick={() => {
                          ...
                          setTimeout(() => setResetDialogOpen(false), 0);
                        }}
                        className="flex-1"
                      >
                        Yes, Reset Form
