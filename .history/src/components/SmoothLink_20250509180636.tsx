  ...
  const handleClick = useCallback((e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    if (needsScroll) {
      ...
      if (!document.body.hasAttribute('data-animating-hero')) {
        ...
        // Only animate hero content if we're in a hero section (check parent elements)
        const heroElements = document.querySelectorAll('[data-hero-content]');
        heroElements.forEach(el => {
          // Check if this element is inside a hero section
          let isInHero = false;
          let parent = el.parentElement;
          while (parent) {
            if (parent.classList.contains('hero-section') ||
                parent.tagName === 'HEADER' ||
                parent.hasAttribute('data-hero-container')) {
              isInHero = true;
              break;
            }
          ...
          if (isInHero && el instanceof HTMLElement) {
