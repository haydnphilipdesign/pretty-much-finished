    ...
    if (pdfBuffer) {
      ...
      if (pdfBuffer.length > MAX_PDF_SIZE_BYTES) {
        ...
        try {
            ...
            transactionFields[transactionFieldMap.pdfAttachment] = [
              {
                ...
                url: `data:application/pdf;base64,${compressedPdf.toString('base64')}`
              }
            ];
          ...
          // Add a note about the PDF compression error
          if (!transactionFields[transactionFieldMap.additionalNotes]) {
            transactionFields[transactionFieldMap.additionalNotes] = '';
          }
          transactionFields[transactionFieldMap.additionalNotes] += `\n\nNOTE: PDF attachment could not be processed due to an error. Please contact the agent for the PDF document.`;
        }
      } else {
