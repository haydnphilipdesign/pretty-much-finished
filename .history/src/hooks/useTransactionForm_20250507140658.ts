...
export const useTransactionForm = () => {
  ...
  const handleNext = () => {
    ...
    if (Object.keys(errors).length === 0 || bypassValidationMode) {
      ...
      // Store the validation errors and show the validation UI
  ...
  // Handle continuing with validation errors
  const handleContinueWithErrors = () => {
    if (Object.keys(validationErrors).length > 0) {
      // Mark fields as skipped
      ...
      // Call handleNext with a slight delay to ensure state is updated
  ...
  const handleFixValidationError = (field: string) => {
    // Find the element with the specified field and scroll to it
    const errorElement = document.querySelector(`[aria-invalid="true"], #${field}`);
