...
export function useClientValidation() {
  ...
  const validateSecondClient = useCallback((formData: TransactionFormData): ValidationResult => {
    // If any second client field is filled, all become required
    ...
    // Validate all required fields for second client
    ...
    if (!formData.secondClientEmail) {
      return { isValid: false, state: 'invalid', error: 'Second client email is required when any second client information is provided' };
    }

    if (!formData.secondClientPhone) {
      return { isValid: false, state: 'invalid', error: 'Second client phone is required when any second client information is provided' };
    }
