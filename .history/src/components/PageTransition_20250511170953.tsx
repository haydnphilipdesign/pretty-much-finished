  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      const heroContentChanged = mutations.some(mutation => {
        return mutation.target.closest('[data-hero-content]') ||
               mutation.target.querySelector('[data-hero-content]');
      });

      if (heroContentChanged && window.pageYOffset > 0) {
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible' && window.pageYOffset > 0) {
        smoothScrollToTop();
      }
    };
