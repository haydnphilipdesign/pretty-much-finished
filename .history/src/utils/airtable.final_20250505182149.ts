...
export async function submitToAirtable(data: TransactionFormState, pdfBuffer?: Buffer) {
  ...
  try {
    ...
    if (pdfBuffer) {
      ...
      try {
        // Handle PDF data based on its type
        let pdfSize: number;
        let pdfBase64: string;
        
        // Convert pdfBuffer to base64 if it's not already
        if (typeof pdfBuffer === 'string') {
          // If it's already a base64 string
          pdfBase64 = pdfBuffer;
          // Estimate size: base64 string length * 0.75 gives approximate byte size
          pdfSize = Math.ceil(pdfBuffer.length * 0.75);
        } else if (pdfBuffer instanceof Uint8Array || pdfBuffer instanceof ArrayBuffer) {
          // Convert ArrayBuffer or Uint8Array to base64
          const binary = Array.from(new Uint8Array(pdfBuffer instanceof ArrayBuffer ? pdfBuffer : pdfBuffer.buffer))
            .map(byte => String.fromCharCode(byte))
            .join('');
          pdfBase64 = btoa(binary);
          pdfSize = pdfBuffer instanceof ArrayBuffer ? pdfBuffer.byteLength : pdfBuffer.length;
        } else if (typeof Buffer !== 'undefined' && pdfBuffer instanceof Buffer) {
          // Node.js Buffer
        ...
        if (pdfSize > MAX_PDF_SIZE_BYTES) {
          console.warn(`PDF size (${pdfSize} bytes) exceeds the maximum allowed size for direct attachment (${MAX_PDF_SIZE_BYTES} bytes).`);
          ...
          try {
            ...
            if (compressedPdf) {
              ...
              if (compressedPdf instanceof Uint8Array) {
                const binary = Array.from(compressedPdf)
                  .map(byte => String.fromCharCode(byte))
                  .join('');
                compressedBase64 = btoa(binary);
              } else if (typeof compressedPdf === 'string') {
                compressedBase64 = compressedPdf;
              } else {
                console.error('Unsupported compressed PDF format');
                handleLargePdfFallback();
                return;
              }
              
              console.log(`Successfully compressed PDF from ${pdfSize} to ${compressedPdf.length || compressedBase64.length * 0.75} bytes`);
              
              // Add the compressed PDF as an attachment
              transactionFields[transactionFieldMap.pdfAttachment] = [
                {
                  filename: pdfFilename,
                  url: `data:application/pdf;base64,${compressedBase64}`
                }
              ];
              
              console.log(`Added compressed PDF attachment to transaction fields`);
            } else {
    ...
    try {
      transactionRecord = await base('Transactions').create({ fields: transactionFields });
    } catch (apiError: any) {
