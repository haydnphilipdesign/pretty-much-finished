...
// Services data with glass card styles
const services = [
  {
    icon: FileSearch,
...
const Services: React.FC = () => {
  ...
  return (
    <div className="min-h-screen">
      ...
      <ContentSection dark={false} className="py-24 relative overflow-hidden" centered={true}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <ContentCard
              ...
              withAnimation={true}
              hoverEffect="lift"
              cardStyle={service.style === "glass-card-navy" ? "glass-navy" : 
                         service.style === "glass-card-blue" ? "glass-blue" : 
                         service.style === "glass-card-gold" ? "glass-gold" :
                         service.style === "glass-card-dark" ? "glass-dark" :
                         service.style === "glass-card-frost" ? "glass-frost" : "glass"}
              iconContent={
                <div className={`w-16 h-16 ${service.style.includes('navy') || service.style.includes('blue') || service.style.includes('dark') || service.style.includes('gold') ? 'bg-white/20' : 'bg-gradient-to-br from-brand-gold/20 to-brand-blue/20'} rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
                  <service.icon className={`w-8 h-8 ${service.style.includes('navy') || service.style.includes('blue') || service.style.includes('dark') || service.style.includes('gold') ? 'text-white' : 'text-brand-gold group-hover:text-brand-blue'} transition-colors duration-300`} />
                </div>
              }
              className="h-full"
            >
              <p className={`${service.style.includes('navy') || service.style.includes('blue') || service.style.includes('dark') || service.style.includes('gold') ? 'text-white/90' : 'text-gray-600'} mb-6 text-sm glass-card-content`}>
                {service.description}
              </p>
              <ul className="space-y-2 mt-auto">
                {service.features.map((feature, i) => (
                  <li key={i} className={`flex items-start ${service.style.includes('navy') || service.style.includes('blue') || service.style.includes('dark') || service.style.includes('gold') ? 'text-white/90' : 'text-gray-600'} text-sm`}>
                    <div className={`w-1.5 h-1.5 ${service.style.includes('navy') || service.style.includes('blue') || service.style.includes('dark') ? 'bg-white' : 'bg-brand-gold'} rounded-full mr-2 mt-1.5`} />
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
      ...
      >
        ...
        <div className="text-center max-w-4xl mx-auto relative z-10">
          ...
          <HeroButton
            ...
            variant="inverted"
            size="lg"
            className="glass-card bg-white hover:bg-white/90 text-blue-600 py-2 rounded-full p-0 px-8 py-4"
            icon={
              <motion.div
                animate={{ x: [0, 5, 0] }}
