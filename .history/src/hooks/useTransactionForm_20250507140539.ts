...
export const useTransactionForm = () => {
  ...
  const handleNext = () => {
    ...
    if (Object.keys(errors).length === 0 || bypassValidationMode) {
      ...
      // Create a toast with our MissingFieldsNotification component
      toast({
        title: "Missing Information",
        description: `${errorCount} required field${errorCount !== 1 ? 's' : ''} ${errorCount !== 1 ? 'are' : 'is'} missing.`,
        variant: "destructive",
        duration: 8000,
        action: () => {
