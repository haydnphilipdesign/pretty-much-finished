...
// Import PDF positions helper - use the .cjs extension for CommonJS compatibility
const pdfPositions = require('../utils/pdfPositions.cjs');
...
function sanitizeText(text) {
    ...
    // Replace other potentially problematic unicode whitespace characters
    return text
        ...
        .replace(/[^\x00-\x7F]/g, char => {
            // For any non-ASCII characters, try to replace with closest ASCII equivalent
            // or replace with a space if no good replacement
            try {
...
/**
 * Maps form data to the PDF positions based on the pdfPositions.js mapping
 * @param {Object} formData - The form data
 * @returns {Array} - Array of text elements with their positions
 */
function mapFormDataToPdfPositions(formData) {
    // Create a structured object that matches what pdfPositions.js expects
    const mappedData = {
        propertyData: {
            address: formData.propertyData && formData.propertyData.address || '',
            mlsNumber: formData.propertyData && formData.propertyData.mlsNumber || '',
            salePrice: formData.propertyData && formData.propertyData.salePrice || '',
            closingDate: formData.propertyData && formData.propertyData.closingDate || '',
            // Don't include status fields as text values
        },
        agentData: {
            name: (formData.agentData && formData.agentData.name) ||
                (formData.agentData && formData.agentData.agentName) ||
                (formData.signatureData && formData.signatureData.agentName) || '',
            role: formData.agentData && formData.agentData.role || ''
        },
        commissionData: {
            totalCommission: formData.commissionData && formData.commissionData.totalCommission || '',
            listingAgentCommission: formData.commissionData && formData.commissionData.listingAgentCommission || '',
            buyersAgentCommission: formData.commissionData && formData.commissionData.buyersAgentCommission || '',
            brokerFeeAmount: formData.commissionData && formData.commissionData.brokerFeeAmount || '',
            buyerPaidAmount: (formData.commissionData && formData.commissionData.buyerPaidAmount) ||
                (formData.commissionData && formData.commissionData.buyerPaidCommission) || '',
            sellersAssist: formData.commissionData && formData.commissionData.sellersAssist || '',
            isReferral: formData.commissionData && formData.commissionData.isReferral || '',
            referralParty: formData.commissionData && formData.commissionData.referralParty || '',
            referralFee: formData.commissionData && formData.commissionData.referralFee || ''
        },
        clients: formData.clients || [],
        propertyDetails: {
            attorneyName: formData.propertyDetails && formData.propertyDetails.attorneyName || '',
            municipality: formData.propertyDetails && formData.propertyDetails.municipality || '',
            hoaName: formData.propertyDetails && formData.propertyDetails.hoaName || ''
        },
        titleData: {
            titleCompany: formData.titleData && formData.titleData.titleCompany || ''
        }
    };

    // Use the imported pdfPositions.js mapFormDataToPositions function
    return pdfPositions.mapFormDataToPositions(mappedData);
}
...
async function fillPdfForm(formData) {
    try {
        ...
        const textElements = mapFormDataToPdfPositions(formData);
        console.log(`Adding ${textElements.length} text elements to PDF`);

        // Add all text elements to the PDF
        textElements.forEach(element => {
            const pageIndex = element.page || 0;
            if (pageIndex >= pages.length) {
                console.warn(`Skipping element for page ${pageIndex} as PDF only has ${pages.length} pages`);
                return;
            }

            const page = pages[pageIndex];
            const fontToUse = element.isBold ? fontBold : font;

            page.drawText(sanitizeText(element.text), {
                x: element.x,
                y: element.y,
...
async function sendPdfEmail(pdfBuffer, formData) {
    try {
        ...
        https: //www.parealestatesupport.com/agent-portal/transactionconst propertyAddress = (formData.propertyData && formData.propertyData.address) || 'Unknown Property';
        ...
        const accessType = (formData.propertyData && formData.propertyData.accessType) || 'Not specified';
        const lockboxAccessCode = (formData.propertyData && (formData.propertyData.accessCode || formData.propertyData.lockboxAccessCode)) || 'Not provided';
            ...
            <div class="section">
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td style="padding: 10px; border-bottom: 1px solid #eee;">${accessType}</td>
                    </tr>
                    <tr>
