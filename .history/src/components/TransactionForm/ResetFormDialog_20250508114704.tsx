...
export const ResetFormDialog: React.FC<ResetFormDialogProps> = ({ onReset }) => {
  ...
  // Ensure dialog closes on page refresh or navigation
  useEffect(() => {
    // Use a cleanup function to ensure dialog state is cleaned up
    return () => {
      setIsOpen(false);
    };
  }, []);
  ...
  const handleReset = () => {
    ...
    onReset();
    // Then close the dialog immediately after
    setIsOpen(false);
    
    // Force close by using a very short timeout as a fallback
    setTimeout(() => {
      setIsOpen(false);
    }, 50);
  };
