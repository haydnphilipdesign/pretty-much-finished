...
export const verifyAddress = async (address: AddressComponents): Promise<VerifiedAddress> => {
  ...
  if (import.meta.env.VITE_USE_MANUAL_ADDRESS_VALIDATION === 'true') {
  ...
  try {
    ...
    const xml = `
      <AddressValidateRequest USERID="${import.meta.env.VITE_USPS_USER_ID}">
        <Revision>1</Revision>
        <Address ID="0">
    ...
    // Call USPS API
    const response = await fetch(`${import.meta.env.VITE_USPS_API_URL}?API=Verify&XML=${encodeURIComponent(xml)}`);
