...
interface CustomValidationDialogProps {
  errorCount: number;
  onContinue: () => void;
...
export const CustomValidationDialog: React.FC<CustomValidationDialogProps> = ({
  errorCount,
  onContinue,
  onFix,
  errors,
  ...
  // Ensure dialog state is cleaned up on component unmount
  useEffect(() => {
    return () => {
      // We no longer auto-close on unmount as this caused issues
      // The dialog should only close via explicit user actions
    };
  }, [isOpen, onClose]);
  
  // Flag to prevent duplicate field fix attempts
  const [hasClickedFix, setHasClickedFix] = useState(false);
  ...
  const handleFixClick = () => {
    ...
    if (hasClickedFix) return;
    setHasClickedFix(true);
    
    // Add a window hash to track that we're fixing fields
    window.location.hash = 'bypassingValidation';
    
    // Call fix function
    onFix(firstErrorField);
    
    // Close dialog with a slight delay to ensure the fix navigation completes first
    setTimeout(() => {
      if (onClose) onClose();
      setHasClickedFix(false);
    }, 100);
  };
  
  const handleContinueClick = () => {
    // Continue function first
    onContinue();
      ...
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-lg w-full relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button 
            ...
            className="flex-1 bg-amber-500 hover:bg-amber-600 text-gray-900 py-3 px-6 rounded-lg font-bold text-lg transition-colors"
          >
            Continue Anyway
          </button>
          <button 
