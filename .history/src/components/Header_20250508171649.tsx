              ...
              {navItems.map((item, index) => (
                ...
                >
                  <Link
                    ...
                    } ${item.path === '/agent-portal' ? 'px-6 py-2 bg-gradient-to-r from-brand-gold to-brand-blue text-white rounded-lg hover:shadow-lg transition-all duration-300' : ''}`}
                  >
                    {item.icon && <span className="mr-1.5">{item.icon}</span>}
                    <span className="relative z-10">{item.label}</span>
                    {item.path !== '/agent-portal' && (
                      <span className={`absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-brand-gold to-brand-blue transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ${
