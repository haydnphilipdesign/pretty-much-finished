        ...
        if (error) {
            ...
            if (listError || !buckets || buckets.length === 0) {
                ...
                try {
                    ...
                    if (createError) {
                        throw new Error('Could not create bucket and no existing buckets available');
                    }
...
module.exports = async(req, res) => {
    ...
    try {
        ...
        // Make transactionId optional - return early without Airtable update if missing
        if (!transactionId) {
            console.log('No transaction ID provided. Will upload to Supabase only.');

            // Just upload to Supabase storage without Airtable attachment
            const uploadResult = await uploadPdfToSupabase(pdfData, filename);
            ...
            return res.status(200).json({
                success: true,
                message: 'PDF uploaded to Supabase successfully (no Airtable update)',
        ...
        try {
            console.log(`Updating Airtable record ${transactionId} with PDF attachment...`);
            const updatedRecord = await base('Transactions').update(transactionId, updateData);

            if (!updatedRecord) {
                throw new Error('Failed to update Airtable record');
            }

            console.log('Successfully updated Airtable record with PDF attachment');

            return res.status(200).json({
                success: true,
                message: 'PDF uploaded and attached to Airtable record',
                url: result.url,
            ...
            // Still return success with the URL since the Supabase upload worked
            return res.status(200).json({
                success: true,
                message: `PDF uploaded to Supabase but Airtable update failed: ${airtableError.message}`,
                url: result.url,
                bucket: result.bucket,
