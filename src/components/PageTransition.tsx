...
const PageTransition: React.FC<PageTransitionProps> = ({ children }) => {
  ...
  const pageVariants = {
    ...
    animate: {
      ...
      transition: {
        type: 'tween',
        ease: [0.25, 0.1, 0.25, 1.0], // Refined cubic-bezier for luxury feel
        duration: 1.0, // Balanced duration for elegance
        opacity: { duration: 1.2 }, // Slightly longer fade for sophistication
      }
    },
    exit: {
      opacity: 0,
      ...
      transition: {
        type: 'tween',
        ease: [0.25, 0.1, 0.25, 1.0], // Refined cubic-bezier
        duration: 0.8, // Faster exit for crisp transitions
        opacity: { duration: 0.6, ease: [0.4, 0.0, 0.2, 1.0] }, // Quick fade out
      }
    }
  };
