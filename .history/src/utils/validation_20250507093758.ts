...
export const validateStep = (
  ...
  switch (step) {
    ...
    case 1:
      ...
      validateRequired(data.agentData?.name, "name", "Agent name");
    ...
    case 4:
      if (isListingOrDualAgent) {
        ...
        if (data.commissionData?.hasSellersAssist) {
          validateRequired(data.commissionData?.sellersAssistAmount, "sellersAssistAmount", "Seller's assist amount");
        }
      } else {
        validateRequired(data.commissionData?.buyersAgentPercentage, "buyersAgentPercentage", "Buyer's agent commission percentage");
        
        if (data.commissionData?.hasEarnestMoney) {
          validateRequired(data.commissionData?.earnestMoneyAmount, "earnestMoneyAmount", "Earnest money amount");
        }
      }
    ...
    // Review & Submit
    case 8:
      // No additional validation, as this should be a summary step
      break;
