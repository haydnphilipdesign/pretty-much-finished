  ...
  const handleNext = () => {
    ...
    if (currentStep === 6) {
      ...
      if (!documentsData.confirmDocuments) {
        ...
        toast({
          title: "Confirm Document Requirements",
    ...
    if (Object.keys(errors).length === 0 || bypassValidationMode) {
      ...
      if (bypassValidationMode && Object.keys(errors).length > 0) {
        ...
        toast({
          ...
          description: "You can complete missing fields later before final submission.",
          variant: "default",
        });
      ...
      // Show first toast with error message
      toast({
        title: "Missing Information",
        description: `${errorCount} field${errorCount !== 1 ? 's' : ''} need${errorCount === 1 ? 's' : ''} to be filled in.`,
        variant: "destructive",
        duration: 8000,
      });
      
      // Add buttons manually to the DOM
      setTimeout(() => {
        // Create container for buttons
        const btnContainer = document.createElement('div');
        btnContainer.id = 'validation-options';
        btnContainer.className = 'fixed bottom-4 right-4 z-50 flex flex-col gap-2';
        
        // Create continue anyway button
        const continueBtn = document.createElement('button');
        continueBtn.innerText = 'Continue Anyway';
        continueBtn.className = 'px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-md shadow-md';
        continueBtn.onclick = () => {
        ...
        // Create fix fields button
        const fixBtn = document.createElement('button');
        fixBtn.innerText = 'Fix Missing Fields';
        fixBtn.className = 'px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md shadow-md';
        fixBtn.onclick = () => {
