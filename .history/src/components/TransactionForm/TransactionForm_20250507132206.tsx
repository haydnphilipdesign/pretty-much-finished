...
export function TransactionForm() {
  ...
  const handleFixField = (field: string) => {
    // Find which step contains the missing field
    const stepWithField = skippedFields.find(item => 
    ...
    if (stepWithField) {
      ...
      setTimeout(() => {
        ...
        if (fieldElement) {
          ...
          fieldElement.classList.add('animate-pulse', 'border-amber-500');
          setTimeout(() => {
            fieldElement.classList.remove('animate-pulse', 'border-amber-500');
          }, 3000);
        }
      }, 100);
    }
