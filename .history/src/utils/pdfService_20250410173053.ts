...
export const generateTransactionPdf = async (formData: any): Promise<string> => {
  ...
  try {
    ...
    const pdfUrl = createDownloadLink(pdfBlob, fileName);
    console.log('PDF download link created:', pdfUrl);
    ...
    let emailBody = `
      ...
      <p><strong>Note:</strong> A PDF with the complete transaction details has been automatically downloaded to your computer.</p>
      <p>The PDF is named: ${fileName}</p>
    `;
