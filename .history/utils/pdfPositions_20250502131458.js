...
export const formFieldPositions = {
  ...
  page1: {
    ...
    agentName: { x: 96.55, y: 546.91 + Y_OFFSET - 2, fontSize: 11, bold: true },
    agentRole: { x: 269.44, y: 546.91 + Y_OFFSET - 2, fontSize: 11, bold: true },
    
    // Buyer Information section - Move DOWN by half the specified amounts
    buyerName: { x: 143.17, y: 494.06 + Y_OFFSET - 2, fontSize: 11, bold: true },
    buyerPhone: { x: 135.43, y: 467.63 + Y_OFFSET - 2, fontSize: 11 },
    buyerAddress: { x: 81.46, y: 441.20 + Y_OFFSET - 2, fontSize: 11 },
    buyerEmail: { x: 125.10, y: 414.77 + Y_OFFSET - 0, fontSize: 11 },
    ...
    sellerPhone: { x: 420.21, y: 467.63 + Y_OFFSET - 2, fontSize: 11 },
    sellerAddress: { x: 370.00, y: 441.20 + Y_OFFSET - 2, fontSize: 11 }, // Shifted left from 420.00
    sellerEmail: { x: 408.98, y: 414.77 + Y_OFFSET - 0, fontSize: 11 },
    
    // Legal Information section - Move DOWN by 13.9 points (half of 27.8)
    attorneyName: { x: 94.97, y: 361.92 + Y_OFFSET - 2, fontSize: 11 },
    
    // Financial Details section - Move DOWN by half the specified amounts
    listSidePercentage: { x: 113.13, y: 289.24 + Y_OFFSET + 2, fontSize: 11, bold: true },
    buyerSidePercentage: { x: 251.81, y: 289.24 + Y_OFFSET + 2, fontSize: 11, bold: true },
    totalPercentage: { x: 113.13, y: 262.81 + Y_OFFSET + 2, fontSize: 11, bold: true },
    
    // Title Information - Move DOWN by 14.9 points (half of 29.8)
    titleCompany: { x: 420.59, y: 163.71 + Y_OFFSET + 2, fontSize: 11, bold: true },
  ...
  page2: {
    // C/O Additional Information
    municipality: { x: 200, y: 700 + Y_OFFSET, fontSize: 11, bold: true },
    
    // Resale Additional Information
    hoaName: { x: 200, y: 500 + Y_OFFSET, fontSize: 11, bold: true }
  }
};
...
export function mapFormDataToPositions(formData) {
  ...
  if (formData.propertyDetails) {
    ...
    if (formData.propertyDetails.municipality) {
      addTextElement(textElements, 0, formFieldPositions.page2.municipality,
        formData.propertyDetails.municipality);
    }

    // HOA Name
    if (formData.propertyDetails.hoaName) {
      addTextElement(textElements, 0, formFieldPositions.page2.hoaName,
