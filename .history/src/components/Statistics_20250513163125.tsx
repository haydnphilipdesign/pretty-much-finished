        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {stats.map((stat, index) => (
            <ContentCard
              ...
              iconContent={
                <div className="mb-6 bg-[rgba(255,184,28,0.15)] w-32 h-32 rounded-full flex flex-col items-center justify-center backdrop-blur-sm border border-[rgba(255,184,28,0.3)]">
                  {index === 0 ? (
                    <>
                      <span className="text-[#FFB81C] font-bold text-4xl">30+</span>
                      <span className="text-white text-lg mt-1">Years</span>
                    </>
                  ) : (
                    <span className={`text-[#FFB81C] font-bold ${
                      stat.value.length > 5 ? 'text-3xl' : 'text-4xl'
                    }`}>
                      {stat.value}
                    </span>
                  )}
            ...
            >
              ...
              >
                <div className="text-white font-semibold text-xl mb-2 text-center">
                  {index === 0 ? 'of Excellence' : stat.label}
                </div>
                <div className="text-white/80 text-sm text-center">{stat.description}</div>
              </motion.div>
            </ContentCard>
          ))}
        </div>
